...
const About = (props) => {
  ...
  const dataCert =
    _lang === "es" ? props.data.dataCert_es : props.data.dataCert_en;
              ...
              return (
                <li key={item.node.id} className="divLiCert">
                  <div className="imgCertCont">
                    <Img
                      fluid={item.node.certificationLogo.fluid}
                      className="imgCert"
                    />
                  </div>
                  <hr className="hrAbout" />
                  <p className="subtitleAbout boldText">
                    Certification: {item.node.title}
                  </p>
                  <p className="subtitleAbout">
                    Issued Date: {item.node.issuedDate}
                  </p>
                  <div
                    className="textMiniAbout"
                    dangerouslySetInnerHTML={{
                      __html:
                        item.node.descriptionNode.childMarkdownRemark.html,
                    }}
                  />
                  <p className="subtitleAbout">
                    Expertise: {item.node.areaExpertise}
                  </p>
                </li>
              );
            })}
...
export const data = graphql`
  query {
    ...
    dataCert_en: allDatoCmsCertification(
      filter: { locale: { eq: "en" } }
      sort: { order: DESC, fields: issuedDate }
    ) {
      edges {
        node {
          areaExpertise
    ...
    dataCert_es: allDatoCmsCertification(
      filter: { locale: { eq: "es" } }
      sort: { order: DESC, fields: issuedDate }
    ) {
      edges {
